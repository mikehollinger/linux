...
retry:
	...
	scsi_cmd[0] = SERVICE_ACTION_IN;	/* read cap(16) */
			...
			u32 port_sel)
{
	...
	wmb(); /* Make setting of format bit visible */

	rhte_f1->lun_id = lun_id;
	wmb(); /* Make setting of LUN id visible */
	...
	wmb(); /* Make remaining RHT entry fields visible */
}
			   ...
			   struct dk_cxlflash_release *release)
{
	...
	case MODE_PHYSICAL:
		...
		rhte_f1->valid = 0;
		wmb(); /* Make revocation of RHT entry visible */

		rhte_f1->lun_id = 0;
		wmb(); /* Make clearing of LUN id visible */

		rhte_f1->dw = 0;
		wmb(); /* Make RHT entry bottom-half clearing visible */
...
int cxlflash_ioctl(struct scsi_device *sdev, int cmd, void __user *arg)
{
	...
	case DK_CXLFLASH_VLUN_RESIZE:
	case DK_CXLFLASH_VLUN_CLONE:
		dev_dbg(dev, "%s: %s (%08X) on dev(%d/%d/%d/%u)\n",
...
cxlflash_ioctl_exit:
	...
	if (unlikely(rc && known_ioctl))
		dev_err(dev, "%s: ioctl %s (%08X) on dev(%d/%d/%d/%u) "
			"returned rc %d\n", __func__,
			decode_ioctl(cmd), cmd, shost->host_no,
			sdev->channel, sdev->id, sdev->lun, rc);
	else
		dev_dbg(dev, "%s: ioctl %s (%08X) on dev(%d/%d/%d/%u) "
